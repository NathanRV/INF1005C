...
%Suppression ligne supplémentaire
    ...
    t=1;
        while t>0
            ...
            elseif c==2
                ...
                if strcmpi(Paquet{i,num}{2},cn)==1 && strcmpi(Paquet{i,num}{2},'as')==1 || strcmpi(Paquet{i,num}{1},ct)==1 && strcmpi(Paquet{i,num}{2},'as')==1
                    if i<j
                        Paquet{i+1,end+1}{2}={};
                    elseif i==j
                        Paquet{1,end+1}{2}={};
                ...
                elseif strcmpi(Paquet{i,num}{2},'2')==1 && strcmpi(Paquet{i,num}{2},cn)==1 || strcmpi(Paquet{i,num}{1},ct)==1 && strcmpi(Paquet{i,num}{2},'2')==1
                ...
                %Sinon pige 2 cartes
                    ...
                    %1ere carte
                    M=randi([1,4]);
                    while isempty(Paquet{i,end-1}{2})==1
                        F=0;
                        N=randi([1,13]);
                    ...
                    %2ieme carte
                    M=randi([1,4]);
                    while isempty(Paquet{i,end}{2})==1
                        F=0;
                        N=randi([1,13]);
                        ...
                        if isempty(Paquet{i,end}{2})==1
                            fprintf('La carte que vous avez joué n''est pas valide. La pioche est vide!');
                            Paquet{i,end}=[];
                        else
                            fprintf('La carte que vous avez joué n''est pas valide. Pénalité de 2 cartes.');
                        end 
...
%Assigne cellules vides pour pouvoir afficher comptage
for x=1:j
    for y=1:size(Paquet,2)
        if isempty(Paquet{x,y})==1
            Paquet{x,y}{2}={'vide'};
        end
    end
end
...
for x=1:j
    ...
    for y=1:size(Paquet,2)
        ...
        elseif strcmpi(Paquet{x,y}{2},'9')==1
            Pts(y)=9;
        elseif strcmpi(Paquet{x,y}{2},'10')==1
            Pts(y)=10;
        end
    end
    A(x)=sum(Pts);
